...
import { observeSidebarAllAccountsBalance, refreshPrivacyMode } from "./privacy-utils";
...
(function () {
	...
	function buildTemplateRow(t, entry) {
		...
		if (t.scheduleName) {
			...
			return el("li", { class: "abt-cti-template abt-cti-template-schedule abt-cti-template-missing" }, [
				priority,
				body,
			]);
		}
	...
	function openScheduleModal(schedId) {
		...
		const maxTries = 40;
		const step = () => {
			const row = document.querySelector(`[data-focus-key="${schedId}"] [data-testid="row"]`);
			if (row) {
